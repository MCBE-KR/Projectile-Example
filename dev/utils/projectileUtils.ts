...
const getDiffVector = (startLocation: Vector3, endLocation: Vector3) => {
    return {
...
const addHitBlocks = (startLocation: Vector3, endLocation: Vector3, hitLocations: Vector3[]) => {
    ...
    if (totalDiff === 0) {
        ...
        if (totalDiff === 2) {
            ...
            let tempDiffVector = getDiffVector(startLocation, firstIntersectionVector);
            if (sumVector(tempDiffVector) !== 1) {
                tempDiffVector = getDiffVector(startLocation, firstIntersectionVector);
                ...
                if (sumVector(getDiffVector(startLocation, firstIntersectionVector)) !== 1) {
                    tempDiffVector = getDiffVector(startLocation, firstIntersectionVector);
                    firstIntersectionVector = getIntersectionVector(
                        startLocation,
                        directionVector,
            ...
            hitLocations.push(firstIntersectionVector);

            const secondIntersectionVector = getIntersectionVector(
            ...
            hitLocations.push(secondIntersectionVector);
        }
    }
};
